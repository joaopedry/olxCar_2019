...
$_SG["link"]->set_charset("utf8");
	...
	function cadastrarUsuario()
	{
		...
		if(isset($_POST['cadastrarUsuario']))
		{
			...
			if(!mysqli_fetch_assoc($queryCadastrarSelect))
			{
				...
				{
					echo '<script language="javascript">';
					echo 'alert("Erro ao cadastrar, por favor tente novamente mais tarde!")';
					echo '</script>';
				}
			}
			else 
			{
				unset($_POST['cadastrarUsuario']);
				echo '<script language="javascript">';
				echo 'alert("Este login já está em uso!")';
				echo '</script>';
			}
		}
	}
	...
	{
		global $_SG;
		if(isset($_POST['cadastrarEstado']))
		{
			$NomeEstado 		= $_POST['txtEstado'];

			$sqlCadastrarEstado = "INSERT INTO `estados` (ds_estado) VALUES ('".$NomeEstado."')";
			
			if (mysqli_query($_SG['link'], $sqlCadastrarEstado))
			{
				unset($_POST['cadastrarEstado']);
	...
	function cadastrarMarca()
	{
		...
		if(isset($_POST['cadastrarMarca']))
		{
			...
			$sqlCadastrarMarca = "INSERT INTO `marcas` (ds_marca) VALUES ('".$NomeMarca."')";
			
			if (mysqli_query($_SG['link'], $sqlCadastrarMarca))
			{
	...
	function cadastrarCor()
	{
		...
		{
			$NomeCor		 = $_POST['txtCor'];

			$sqlCadastrarCor = "INSERT INTO `cores` (ds_cor) VALUES ('".$NomeCor."')";
			
			if (mysqli_query($_SG['link'], $sqlCadastrarCor))
			{
				unset($_POST['cadastrarCor']);
	...
	function atualizarAnuncio()
	{
		...
		if(isset($_POST['atualizarAnuncio']))
		{
			...
			header('location: admin/cadastraAnuncio.php');
		}else
		{
			echo '<script language="javascript">';
